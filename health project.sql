...
select name,avg(age) from healthcare group by name;

-- 6. Identify the patients who have more than one medical condition. 
select name, count(medical_condition) from healthcare group by name
...
select * from healthcare where medical_condition = 'diabetes' and age >60 ; 
...
having test_results in ('abnormal','inconclusive');
...
select count(name) from healthcare where medical_condition='arthritis' and gender='male';
...
select name,medical_condition from healthcare where age > (select avg(age) from healthcare);